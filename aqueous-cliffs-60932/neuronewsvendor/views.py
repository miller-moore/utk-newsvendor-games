...
from .models import Constants, profit, trueorderquantity, set_time, blocktitle, blockinstruction
...
class WelcomePage(Page):
    ...
    def vars_for_template(self):
        ...
        blockinstructreap = blockinstruction('Reappraise')
        blockinstructatt = blockinstruction('Attend')
        ...
        return {
            'blockreap': blockinstructreap,
            'blockatt': blockinstructatt,
...
class BlockInstructionPage(Page):

    def is_displayed(self):

        if (self.round_number + 4) % 5 == 0:
            disp = 1
        else:
            disp = 0
    ...
    def vars_for_template(self):
        ...
        return {
            ...
            'blockinstruction': blockinstruct
        }
...
class BlockResults(Page):
    ...
    def vars_for_template(self):
        ...
        return {
            ...
            'demand': demand,
            'demandindex': safe_json(demandindex),
            'player_in_all_rounds': self.player.in_rounds(1, self.player.round_number - 5),
            'player_in_previous_5rounds': self.player.in_rounds(self.player.round_number - 4, self.player.round_number)
        }
